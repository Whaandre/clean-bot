...
class AlphaBetaEngine:
    ...
    # ---------------- Time helpers ----------------
    def _start_timer(self, time_limit):
        self._search_start = time.time()
        self._time_limit = time_limit

    def _time_up(self):
        return time.time() - self._search_start >= self._time_limit
    ...
    def _quiescence(self, board, alpha, beta, ply):
        if self._time_up(): return self._eval_cp(board)
        self.nodes += 1
    ...
    def _alphabeta(self, board, depth, alpha, beta, ply):
        if self._time_up(): return self._eval_cp(board)
        self.nodes += 1
        if depth <= 0: return self._quiescence(board, alpha, beta, ply)

        original_alpha = alpha
        tt_score, tt_move = self._tt_probe(board, depth, alpha, beta)
        if tt_score is not None: return tt_score
        if tt_move not in board.legal_moves: tt_move = None
        ...
        if best_score <= original_alpha:
            ...
            flag = TTEntry.EXACT
        if best_move: self._tt_store(board, depth, best_score, flag, best_move)
    ...
    def _iterative_deepening(self, board, max_depth, time_limit):
        ...
        for depth in range(1, max_depth+1):
            ...
            if abs(score) >= self.MATE_SCORE - 10: break
    ...
    def _allocate_time(self, board, remaining_time):
        ...
        if num_pieces > 20:        # opening
            return min(3, remaining_time * 0.03)
        elif num_pieces > 10:      # middlegame
            return min(7, remaining_time * 0.07)
        else:                      # endgame
            return min(10, remaining_time * 0.10)
